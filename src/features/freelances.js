...
export async function fetchOrUpdateFreelances(store) {
  ...
  try {
   ...
   // console.log(data.freelancersList.filter( freelancer => freelancer.id === '2'));
    // on verifie si un id à eté  reconnu par la function
    ...
    switch (action.type) {
      ...
      case ID: {
        if(draft.status === 'resolved' || draft.status === 'updating'){
          draft.freelanceID = action.payload
          return
        }
        return
      }
